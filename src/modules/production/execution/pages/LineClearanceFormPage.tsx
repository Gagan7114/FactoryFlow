...
import {
  ...
  useCreateLineClearance,
  useLineClearanceDetail,
  useLines,
  useRunDetail,
  useSubmitLineClearance,
  useUpdateLineClearance,
} from '../api';
...
function LineClearanceFormPage() {
  ...
  // Local checklist changes are only saved when the user clicks "Save".
  const [itemDraftResults, setItemDraftResults] = useState<Record<number, ClearanceResult>>({});
  const localItems = useMemo(
    () =>
      clearance?.items.map((item) => ({
        id: item.id,
        result: itemDraftResults[item.id] ?? item.result,
        remarks: item.remarks,
      })) ?? [],
    [clearance?.items, itemDraftResults],
  );
  const hasUnsavedChanges = Object.keys(itemDraftResults).length > 0;
  ...
  const handleCreate = async () => {
    try {
      ...
      toast.error(getErrorMessage(error, 'Failed to create clearance'));
    }
  };

  const handleLocalItemChange = (itemId: number, result: ClearanceResult) => {
    setItemDraftResults((prev) => ({ ...prev, [itemId]: result }));
  };
      ...
      {isNew ? (
        <>
          <DashboardHeader
            ...
            description={
              linkedRun
                ? `For Run #${linkedRun.run_number} — ${linkedRun.line_name} — ${linkedRun.product}`
                : 'Create a pre-production line clearance checklist'
            }
          />
          <Card className="max-w-lg">
            <CardContent className="p-6 space-y-4">
              ...
              <Button
                ...
                disabled={(!linkedRun && !lineId) || createClearance.isPending}
              >
        ...
        <>
          <div className="flex items-center gap-3">
            <DashboardHeader
              ...
              description={
                clearance.run_number
                  ? `Run #${clearance.run_number} · ${clearance.line_name} · ${clearance.date}`
                  : `${clearance.line_name} · ${clearance.date}`
              }
            />
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                ...
                <SignatureBlock
                  ...
                  sign={clearance.qa_approved ? 'Approved' : ''}
                  signedAt={clearance.qa_approved_at}
                />
              </div>
            </CardContent>
          </Card>
                ...
                <Button
                  ...
                  title={
                    hasUnsavedChanges
                      ? 'Save changes first'
                      : !allItemsFilled
                        ? 'Fill all checklist items first'
                        : undefined
                  }
                >
