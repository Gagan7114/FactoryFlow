import { CheckCircle2, Clock, FileText, LogIn, Plus, RefreshCw, Search, Truck, User } from 'lucide-react';
import { useMemo, useState } from 'react';
import { useNavigate } from 'react-router-dom';

import { useGlobalDateRange } from '@/core/store/hooks';
import { type BSTGateInEntry, useBSTGateInEntries } from '@/modules/gate/api';
...
export default function BSTInDashboardPage() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useBSTGateInEntries(queryParams);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">BST In</h2>
          <p className="text-muted-foreground">
            Receive in-transit branch stock transfer vehicles at the destination gate
          </p>
        </div>
        <div className="flex w-full flex-col gap-3 sm:flex-row lg:w-auto">
          ...
          <Button onClick={() => navigate('/gate/bst-in/new')}>
      ...
      <section>
        ...
        {isLoading ? (
          <EmptyState text="Loading BST in entries..." />
        ) : entries.length === 0 ? (
          <EmptyState text="No BST in entries in this date range" />
        ) : filteredEntries.length === 0 ? (
          <EmptyState text="No BST in entries match this search" />
        ) : (
                  ...
                  {filteredEntries.map((entry) => {
                    ...
                    return (
                      <tr
                        ...
                        onClick={() => {
                          if (!isCancelled) {
                            navigate(getBSTInResumePath(entry));
                          }
                        }}
                      >
